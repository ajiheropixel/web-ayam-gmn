...
<body class="bg-white text-slate-900">
    ...
    <section id="beranda" class="pt-32 pb-20 px-6 bg-gradient-to-b from-orange-50 to-white">
        <div class="max-w-7xl mx-auto grid md:grid-cols-2 gap-12 items-center">
            <div>
                <span class="bg-white text-orange-600 px-4 py-2 rounded-full text-xs font-black uppercase tracking-widest shadow-sm italic">Premium Farm</span>
                ...
                <div class="mt-10 flex gap-4">
                    ...
                    <a href="https://wa.me/{{ $settings['contact_wa'] ?? '6281291502925' }}" class="bg-white border border-slate-200 text-slate-700 px-8 py-4 rounded-2xl font-bold hover:bg-slate-50 transition-all">Chat WhatsApp</a>
                </div>
            </div>
            <div class="relative">
    ...
    <section class="py-24 bg-slate-50">
        <div class="max-w-7xl mx-auto px-6">
            <h2 class="text-center text-4xl font-black mb-16 italic">Suara Pelanggan</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div class="bg-white p-10 rounded-[40px] border border-slate-100 shadow-sm">
                    <p class="text-lg text-slate-600 italic mb-8">"{{ $settings['testi_content_1'] ?? 'Dagingnya segar sekali, beda dengan di pasar!' }}"</p>
                    <h4 class="font-black text-slate-900 text-xl tracking-tight">- {{ $settings['testi_name_1'] ?? 'Pelanggan Setia' }}</h4>
                </div>
                <div class="bg-white p-10 rounded-[40px] border border-slate-100 shadow-sm">
                    <p class="text-lg text-slate-600 italic mb-8">"{{ $settings['testi_content_2'] ?? 'Pengiriman cepat dan packing sangat rapi.' }}"</p>
                    <h4 class="font-black text-slate-900 text-xl tracking-tight">- {{ $settings['testi_name_2'] ?? 'Pembeli Puas' }}</h4>
                </div>
            </div>
        </div>
    </section>
    ...
    <section id="produk" class="py-24">
        <div class="max-w-7xl mx-auto px-6 text-center mb-16">
            <h2 class="text-4xl md:text-5xl font-black text-slate-900 tracking-tighter">Katalog Ayam <span class="text-orange-600 italic">Fresh.</span></h2>
        </div>
        <div class="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-10">
            @foreach(\App\Models\Product::latest()->get() as $item)
            <div class="bg-white rounded-[45px] overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 group border border-slate-50">
    ...
    <footer id="kontak" class="bg-slate-900 pt-20 pb-10 px-6">
        <div class="max-w-7xl mx-auto grid md:grid-cols-4 gap-12 text-slate-400">
            <div class="md:col-span-2">
                ...
                <p class="max-w-sm leading-relaxed">{{ $settings['contact_address'] ?? 'Jl. Peternakan No. 88, Jakarta Timur' }}</p>
            </div>
            <div>
